...
class FollowingScreen extends StatefulWidget {
  ...
  const FollowingScreen({super.key, required this.userId, required this.username});
...
class _FollowingScreenState extends State<FollowingScreen> {
  ...
  Future<void> _loadFollowing() async {
    try {
      ...
      for (final id in user.following) {
        final u = await _firestoreService.getUser(id);
        if (u != null) {
          following.add(u);
      ...
      if (mounted) setState(() { _currentUserId = currentUserId; _following = following; _followingStatus = status; _isLoading = false; });
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('${widget.username} - ${lang.following}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600))),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _following.isEmpty
              ? Center(child: Text(lang.isKurdish ? 'هیچ شوێنکەوتنێک نییە' : 'Not following anyone yet', style: TextStyle(fontSize: 16, color: Colors.grey[600])))
              : ListView.builder(
